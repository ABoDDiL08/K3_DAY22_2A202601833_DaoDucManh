...
# Guardrails uses OpenTelemetry for anonymous metrics; keep the local demo
# deterministic and offline unless the user explicitly enables a collector.
...
# Validator demo không cần gửi anonymous Guardrails telemetry.
...
class JSONFormatter(Validator):
    ...
    def _repair(text: str) -> str:
        ...
        # Chỉ đổi các chuỗi được bao bởi single quote; cách này không làm hỏng
        # dấu nháy đơn nằm bên trong một chuỗi JSON hợp lệ dùng double quote.
        single_quoted = r"'([^'\\]*(?:\\.[^'\\]*)*)'"
        text = re.sub(
            rf"([{{\[,]\s*){single_quoted}",
    ...
    def validate(self, value: str, metadata: dict):
        ...
        try:
            ...
            return PassResult(value_override=json.dumps(
        ...
        try:
            ...
            return FailResult(
                error_message="JSON đầu vào đã được tự động sửa",
                fix_value=formatted,
            )
